const transactionsFrequency = async (address, transactions) => {
    ...
    const dormant_days = gaps
        .filter((gap) => gap > 7)
        .reduce((sum, gap) => sum + gap, 0);
    const longest_gap_days = Math.max(...gaps);
    ...
    const total_period_days = (lastTimestamp - firstTimestamp) / (1000 * 86400);
    const dormant_percent =
        total_period_days > 0 ? (dormant_days / total_period_days) * 100 : 0;
